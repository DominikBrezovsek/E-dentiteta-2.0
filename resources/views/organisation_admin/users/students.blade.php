...
@section('content')
        ...
        <div class="cards-content">
            <div class="card-header">
                ...
                <a href="{{ route('organisation_admin.student.add')}}"
                   class="btn-add-user">Dodaj uporabnika</a>
            </div>
                        ...
                        @if (count($data) > 0)
                            @foreach ($data as $row)
                                <tr>
                                    ...
                                    <td>
                                        <div class="options">
                                            <a href="{{ route('organisation_admin.student', ['userId' => $row?->id]) }}"
                                            ...
                                            <form
                                                ...
                                                <button type="submit" class="btn-delete"
                                                        onclick="deleteStudent(event, this.parentNode)">
                                                    Izbriši
                                                </button>
                                            </form>
                                        </div>
    ...
    <script>
        function deleteStudent(event, form) {
            event.preventDefault();
            @isset($row)
            Swal.fire({
                title: 'Potrditev dejanja',
                text: 'Želite izbrisati dijaka?  Tega dejanja ni mogoče razveljaviti!',
                showDenyButton: true,
                denyButtonText: 'Izbriši dijaka',
