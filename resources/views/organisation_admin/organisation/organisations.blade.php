...
@section('content')
    <div class="card">
        ...
        <a href="{{ route('organisation_admin.professor.add')}}"
                        ...
                        @foreach ($data as $row)
                            <tr>
                                ...
                                @if ($row?->checkking_all_cards == 'Y')
                                    <td><svg xmlns="http://www.w3.org/2000/svg" width="42" height="42"
                                            fill="currentColor" class="bi bi-check text-success" viewBox="0 0 16 16">
                                            <path
                                                d="M10.97 4.97a.75.75 0 0 1 1.07 1.05l-3.99 4.99a.75.75 0 0 1-1.08.02L4.324 8.384a.75.75 0 1 1 1.06-1.06l2.094 2.093 3.473-4.425a.267.267 0 0 1 .02-.022z" />
                                        </svg></td>
                                @else
                                ...
                                @endif
                                <td>{{ $row?->username }}</td>
                                <td><a href="{{ route('organisation_admin.professor', ['organisationId' => $row?->id_organisation]) }}"
                                    class="btn btn-primary">Uredi</a></td>
                                <td>
                                    <form
                                        action="{{ route('organisation_admin.professor.delete', ['organisationId' => $row?->id_organisation]) }}"
                                        method="POST">
                                        @method('DELETE')
                                        @csrf
