...
@section('content')
    <div class="card">
        ...
        <a href="{{ route('admin.organisation.add')}}"
                    ...
                    @if (count($data) > 0)
                        @foreach ($data as $row)
                            <tr>
                                ...
                                @endif
                                <td>{{ $row?->username }}</td>
                                <td><a href="{{ route('admin.organisation', ['organisationId' => $row?->id_organisation]) }}"
                                    class="btn btn-primary">Uredi</a></td>
                                <td>
                                    <form
                                        action="{{ route('admin.organisation.delete', ['organisationId' => $row?->id_organisation]) }}"
                                        method="POST">
                                        @method('DELETE')
                                        @csrf
