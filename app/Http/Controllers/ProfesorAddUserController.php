...
class ProfesorAddUserController extends Controller
{
    public function getUsers()
    {
        $classId = OrganisationAdmin::whereIdUser(session('user')['id'])->first();
        return view(
            'professor.user.users',
            [
                'title' => 'Uporabniki',
                'data' => Students::where('students.id_class', '=', $organisationId->id_organisation)->join('users', 'users.id', '=', 'students.id_user')->get()
            ]
        );
    }
